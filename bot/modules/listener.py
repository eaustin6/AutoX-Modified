...
class MirrorLeechListener:
    def __init__(self, bot, message, isZip=False, extract=False, isQbit=False, isLeech=False, pswd=None, tag=None, select=False, seed=False):
        ...
        self.suproc = None
        self.from_user = self.message.from_user if self.message.sender_chat is None else self.message.sender_chat
    ...
    def onUploadComplete(self, link: str, size, files, folders, typ, name):
        ...
        if self.isLeech:
            ...
            msg += f'\n<b>cc: </b>{self.tag}\n\n'
            msg += f"<b>Time Elapsed:</b> <code>{get_readable_time(time() - self.message.date.timestamp())}</code>\n\n"
            if not files:
